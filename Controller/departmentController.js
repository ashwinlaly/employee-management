...
const getDepartmentList = (req, res) => {
    Department.find({}, '_id name original_name status').then(data => {
        ...
        return res.status(206).json({message: constant.LISTING_DEPARTMENT_ERROR, code: 206})
    })
}
...
const deleteDepartment = (req, res) => {
    _id = req.params.id
    Department.findOneAndDelete(_id, (error, data) => {
        if(_.isEmpty(data)) {
            return res.status(200).json({message: constant.DELETE_DEPARTMENT_SUCCESS, code: 200})
        } else {
            return res.status(206).json({message: constant.DEPARTMENT_REMOVED_ALREADY, code: 206, error})
        }
    })
}
